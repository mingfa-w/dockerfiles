...
docker_run_flag=" -v /tmp/.X11-unix:/tmp/.X11-unix \
                -e DISPLAY=$DISPLAY -e GDK_SCALE -e GDK_DPI_SCALE \
                -v /dev:/dev -v /usr/bin/cnmon:/usr/bin/cnmon \
                --shm-size=20gb --cap-add=SYS_PTRACE "
...
if [ -z "$OLD_ID" ]; then
    CMD="docker run $docker_in_docker $docker_run_flag -it -d --name $CONTAINER_NAME $MOUNT_DIR_ASCEND -v $MOUNT_DIR:/host --tmpfs /tmp:exec --rm $image "
    echo CMD = $CMD
    ID=`$CMD`
